...
class CliArgs:
    ...
    def __post_init__(self):
        ...
        if self.dataset == "mt-bench":
            supported_mt_bench_modes = ["openjury", "fastchat"]
            supported_mt_bench_turns = ["both", "single", "multi"]
            assert self.mt_bench_compatibility in supported_mt_bench_modes, (
                f"Only {supported_mt_bench_modes} are supported but got "
                f"{self.mt_bench_compatibility}."
            )
            assert self.mt_bench_turns in supported_mt_bench_turns, (
                f"Only {supported_mt_bench_turns} are supported but got "
                f"{self.mt_bench_turns}."
            )
    ...
    def parse_args(cls):
        ...
        parser.add_argument(
            "--dataset",
            help="The dataset to use. For instance `alpaca-eval`, `arena-hard`, `m-arena-hard-EU`, `mt-bench`, "
            "`mt-bench-101` for instruction tuning cases or `french-contexts`, `spanish-contexts` for base models.",
        )
        parser.add_argument(
            "--model_A",
            required=True,
        ...
        mt_bench_group = parser.add_argument_group(
            "MT-Bench (original) options",
            "These options only apply when --dataset is mt-bench.",
        )
        mt_bench_group.add_argument(
            "--mt_bench_turns",
            type=str,
...
def _save_mt_bench_results(
    *,
    ...
    if result_name is not None and name_suffix is not None:
        raise ValueError("Provide only one of result_name or name_suffix.")
    name = (
        result_name
        if result_name is not None
        else _build_mt_bench_result_name(args, suffix=name_suffix)
    )
    res_folder = Path(args.result_folder) / name
    res_folder.mkdir(parents=True, exist_ok=True)
